...
IPSET_FILE="/opt/etc/nfqws/ipset.list"
EXCLUDE_FILE="$TMP_DIR/exclude.tmp"
FINAL_FILE="$TMP_DIR/final_list.tmp"

mkdir -p "$TMP_DIR"

rm -f "$LIST_FILE" "$IPSET_FILE" "$EXCLUDE_FILE" "$FINAL_FILE"
...
curl -k -s "https://raw.githubusercontent.com/TripleA150/All-in/refs/heads/main/ipset.lst" > "$IPSET_FILE"
